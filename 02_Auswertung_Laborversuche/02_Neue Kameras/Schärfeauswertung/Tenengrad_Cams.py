...
OUT_BASE  = BASE_ROOT / "_Auswertung_Tenengrad"          
...
for cam in CAM_FOLDERS:
    ...
    plt.ylabel(f"Tenengrad (Gradientenenergie) [-] ({MAIN_LABEL})")
    plt.title(f"{cam}: Tenengrad vs. Frequenz (ROI {int(ROI_FRAC*100)}% zentral)")
    ...
    for signal in ["Sinus", "Rechteck"]:
        ...
        plt.title(f"{cam}: {signal} – Tenengrad vs. Frequenz")
        plt.grid(True, alpha=0.3)
    ...
    pivot = df.pivot_table(index="frequency_hz", columns="signal",
                           values=("median_tenengrad" if USE_MEDIAN_AS_MAIN else "mean_tenengrad"),
                           aggfunc="first").reset_index()

    pivot_csv = OUT_DIR / f"tenengrad_pivot_{'median_tenengrad' if USE_MEDIAN_AS_MAIN else 'mean_tenengrad'}.csv"
    ...
    print(f"\n[DONE] {cam}: Tenengrad-Auswertung abgeschlossen. Ergebnisse in: {OUT_DIR}\n")


# alle Kameras gemeinsame CSV + Plots
if not all_cam_dfs:
...
df_all.to_csv(combined_dir / "tenengrad_summary_all_cams.csv", index=False)
print(f"[OK] Combined CSV: {combined_dir / 'tenengrad_summary_all_cams.csv'}")
...
plt.xlabel("Anregungsfrequenz [Hz]")
plt.ylabel(f"Tenengrad [-] ({MAIN_LABEL})")
plt.title("Alle Kameras: Tenengrad vs. Frequenz (Sinus & Rechteck)")
plt.grid(True, alpha=0.3)
plt.legend(ncol=2, fontsize=9)
export_plot(combined_dir / "ALL_A_tenengrad_vs_frequency_allcams.png")
...
for signal in ["Sinus", "Rechteck"]:
    ...
    plt.xlabel("Anregungsfrequenz [Hz]")
    plt.ylabel(f"Tenengrad [-] ({MAIN_LABEL})")
    plt.title(f"Alle Kameras: {signal} – Tenengrad vs. Frequenz")
    plt.grid(True, alpha=0.3)
    plt.legend()
    export_plot(combined_dir / f"ALL_B_{signal.lower()}_tenengrad_vs_frequency.png")
...
plt.grid(True, alpha=0.3)
plt.legend(ncol=2, fontsize=9)
export_plot(combined_dir / "ALL_C_tenengrad_normalized_allcams.png")
